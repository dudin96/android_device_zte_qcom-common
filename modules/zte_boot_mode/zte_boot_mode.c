 ...
 *
 *   Date         Author      Comment
 *   01/03/2009   zte    Creat initial version
 *   17/04/2009   zte    Support Charge Only Mode
 *   
 */
...
#define ZTE_PU_REASON_CHARGE_ONLY    "0x00000100"
#define ZTE_PU_REASON_CPCAP_RESET    "0x00040000"
#define ZTE_PU_REASON_KPANIC 	      "0x00020000"
#define ZTE_PU_REASON_WDRESET        "0x00008000"
#define ZTE_CID_RECOVER_BOOT	      "0x01"
#define ZTE_DATA_12M		      "1"
...
int check_abnormal_reboot(void)
{
	...
	(!strncmp(abnormal_boot, "wdcpcap", 7))) {
        ALOGD("ZTE_PUPD: bootmode=%s\n", abnormal_boot);
	return 1;
    } else {
	if((check_bootinfo("POWERUPREASON", ZTE_PU_REASON_KPANIC)) ||
	(check_bootinfo("POWERUPREASON", ZTE_PU_REASON_WDRESET)) ||
	(check_bootinfo("POWERUPREASON", ZTE_PU_REASON_CPCAP_RESET))) {
		return 1;
	} else {
		return 0;
	}
